...
class ShuffleNet(nn.Module):
    def __init__(self, cfg, num_classes=10, in_ch=3, in_planes=24):
        super(ShuffleNet, self).__init__()
        out_planes = cfg['out_planes']
        num_blocks = cfg['num_blocks']
        groups = cfg['groups']

        self.conv1 = nn.Conv2d(in_ch, in_planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(in_planes)
        self.in_planes = in_planes
        self.layer1 = self._make_layer(out_planes[0], num_blocks[0], groups)
        self.layer2 = self._make_layer(out_planes[1], num_blocks[1], groups)
        self.layer3 = self._make_layer(out_planes[2], num_blocks[2], groups)
        self.layer4 = self._make_layer(out_planes[3], num_blocks[3], groups)
...
def Shuffle32(**kwargs):
    cfg = {
        'out_planes': [240, 480, 960, 1920],
        'num_blocks': [4, 8, 8, 4],
        'groups': 4
    }
    return ShuffleNet(cfg, in_ch=32, in_planes=64, **kwargs)
